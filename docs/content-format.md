...
`blocks.json` темы говорит, что блок **может** держать и сколько. Раскладка говорит, что он держит
на самом деле: `{ "type": "section", "elements": ["text", "list", "cards"] }` — ровно эти элементы,
ровно в этом порядке. Без `elements` набор и порядок выбирает модель, из того, что тема разрешает.

Последовательность проверяется по тем же диапазонам, что и числа: элемента, которого у блока нет, в
ней быть не может, и количество каждого должно попадать в диапазон темы. Прогон останавливается до
первого платного запроса и называет элемент.

Когда раскладка назвала последовательность у всех блоков одного типа, модель о них не спрашивают
вовсе — она пишет только заголовок, о чём блок и куда ссылаться.
...
относится. Это свойство блока в `templates/<id>/blocks.json`, а не выбор модели: где картинка на
странице, решает оформление, а не текст.
