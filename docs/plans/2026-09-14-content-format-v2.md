...
- Работа идёт в отдельном рабочем дереве, на его собственной ветке. После каждого коммита —
  `git push` (ветка уже связана с `origin`). В `main` ветка вливается после всех задач, не раньше.
...
## Task 1: Адрес страницы — из имени файла
...
## Task 2: Реестр картинок `src/lib/images.mjs`
...
## Task 3: Приведение `content` страницы — `src/lib/content.mjs`
...
## Task 4: Все блоки на `type` + `content`
...
## Task 6: Конвертер таблицы и импорт на новый формат
...
3. Заменить функции `slugFor` и `assignTargets` вместе с комментарием над `assignTargets` на (исправлено по ревью: `home.json` зарезервирован за первым листом главной, лист, лишь похожий на Home, его не займёт):
