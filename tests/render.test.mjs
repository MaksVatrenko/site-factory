...
describe('normalizeSlug holds as an invariant across a wide range of hostile slugs, not six special cases (final-fix-3)', () => {
  ...
  const hostilePages = [
    ...
    { label: 'collides with a file the public folder copies to the output root', slug: '/images/logo.svg' },
  ...
  it('builds successfully and accounts for every page, hostile or not', () => {
    ...
    writeSiteDirFromContent(dir, allContent);

    // One hostile page (below) is a slug that collides with a real publicDir asset — this needs
    // an actual file there to collide with, so this test ships its own small publicDir rather
    // than relying on whatever the default site happens to have (or not have).
    const publicDir = join(dir, 'public');
    mkdirSync(join(publicDir, 'images'), { recursive: true });
    writeFileSync(join(publicDir, 'images', 'logo.svg'), '<svg/>');
...
// exactly like one of those reserved names (Astro itself handles that shape fine — see astro's own
// "Skipping ... because a file with the same name exists in the public folder" behaviour) —
// mirroring it SECOND meant the copy collided with the marker this module had already written
// there itself (cpSync -> ENOTDIR), and that exception took the ENTIRE prober down, not just
// refused this one name. With no prober, normalizeSite silently fell back to the old predictive
// path, which has no rule at all for an unassigned Unicode code point — so a completely unrelated
// hostile page in the SAME build then crashed `astro build` outright, reintroducing the exact
// failure the prober exists to prevent. Reproduced here exactly like that: one publicDir/
// sitemap.xml directory, one page no predictive rule can save, in the same real build.
...
// the finding through a real build, via a temp content folder, per the prescribed method.
